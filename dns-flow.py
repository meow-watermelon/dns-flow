...
def process_payload(packet):
    ...
    if packet.haslayer(DNS):
        ...
        if dns_qr == 0:
            ...
            print(
                f"{timestamp}\t{dns_upper_layer}\tQUERY\t{dns_id}\t{dns_rcode_text}\t{dns_query_name}\t{dns_query_type_text}"
            )
        ...
        if dns_qr == 1:
            ...
            if dns_rcode == 0:
                ...
                for i in range(dns.ancount):
                    ...
                    print(
                        f"{timestamp}\t{dns_upper_layer}\tRESPONSE\t{dns_id}\t{dns_rcode_text}\t{dns_response_rrname}\t{dns_response_type_text}\t{dns_response_ttl}\t{dns_response_data}"
                    )
            else:
                print(
                    f"{timestamp}\t{dns_upper_layer}\tRESPONSE\t{dns_id}\t{dns_rcode_text}"
                )
...
[UTC TIMESTAMP] [PROTOCOL] RESPONSE [Tx ID] [RCODE TEXT]
    ...
    parser = argparse.ArgumentParser(
        description="DNS Flow Monitor - Scapy Version",
        epilog=epilog,
